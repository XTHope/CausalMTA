...
def calculate_Shapley_values(user_tag, seq_C, seq_cat1_9, predictor,  device = "cuda:0"):
    ...
    permutation_list = list(itertools.permutations(array)) # 生成所有索引的排列组合
    sv_array = np.zeros(len(seq_C)) # 存储夏普利值的0数组
    ...
    for i in range(len(seq_C)):
        ...
        for permutation in permutation_list:
            permutation_ = list(permutation) # 转换为列表
            index = permutation_.index(i) # 获取当前索引在列表的位置
